...
4. **Experimental DeepSeek V4 Vision support** — vendors the exact eleven-commit patch series from [vllm#54566](https://github.com/vllm-project/vllm/pull/54566) at head `1576a46008f2411ec51391710c8886293f7a580f`, plus the exact two-commit follow-up [vllm#54631](https://github.com/vllm-project/vllm/pull/54631) at head `8e5ab918974c7eb04f0e984c88f02ad604c9a148`. The image rebuilds vLLM because #54566 changes both Python and the compiled MoE routing operator; #54631 keeps checkpoint loading one-pass and derives Vision DSpark depth from the trained `dspark_block_size`. Do not pass an architecture override; the config converter selects `DeepseekV4ForConditionalGeneration` when it detects the Vision checkpoint. DSpark can use the checkpoint's trained five-token block width.
...
├── patches/                  # Exact upstream vLLM PR patch series, including #54566 and #54631
