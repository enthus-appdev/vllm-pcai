...
4. **Experimental DeepSeek V4 Vision support** — vendors the exact four-commit patch series from [vllm#54566](https://github.com/vllm-project/vllm/pull/54566) at head `93274397143dfc0135b6b0672859ffcab6e725a4`. The image rebuilds vLLM because that PR changes both Python and the compiled MoE routing operator. Do not pass an architecture override; the PR's config converter selects `DeepseekV4ForConditionalGeneration` when it detects the Vision checkpoint. The upstream PR does not exempt DSpark from generic MTP divisibility validation, so start this experimental checkpoint without speculative decoding rather than carrying a custom semantic patch.
